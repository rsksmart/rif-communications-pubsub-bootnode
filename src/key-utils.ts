...
function testLibp2pkey (password: string) {
  create({ keyType: 'secp256k1' }).then((peer: PeerId) => {
    ...
    const encryptedKey: Buffer = exportKey(peer, password)
    console.log('Your encrypted key is')
    console.log(encryptedKey.toString())

    const decryptedKey = decryptPrivateKey(encryptedKey.toString(), password)
    console.log('Your decrypted key is')
    console.log(decryptedKey.toString('base64'))

    const privKeyArray = new Uint8Array(decryptedKey)
    const pubKey: Uint8Array = secp256k1.publicKeyCreate(privKeyArray)
